...
func syncCollaborators(repo string, org string, collaborators map[string]*collaborator) {
	...
	teamsDiff := difference(getGithubTeamNames(currentTeamsCollaborators), getTeamNames(collaborators))
	...
	collabDiff := difference(getGithubCollaboratorNames(currentCollaborators), getCollaboratorNames(collaborators))
...
func getGithubTeamNames(teams []*github.Team) (res []string) {
	for _, team := range teams {
		res = append(res, *team.Slug)
	}
	return
}

func getTeamNames(teams map[string]*collaborator) (res []string) {
	for name, team := range teams {
		if team.IsTeam != nil && *team.IsTeam {
			res = append(res, name)
		}
	}
	return
...
func getGithubCollaboratorNames(users []*github.User) (res []string) {
	for _, user := range users {
		res = append(res, *user.Login)
	}
	return
}

func getCollaboratorNames(collaborators map[string]*collaborator) (res []string) {
	for name, collaborators := range collaborators {
		if collaborators.IsTeam == nil || !*collaborators.IsTeam {
			res = append(res, name)
		}
	}
	return
